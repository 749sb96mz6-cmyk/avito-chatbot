...
9.1. ЗАПРОС ДРУГИХ ЗАПЧАСТЕЙ ИЛИ ВАРИАЦИЙ:
   ...
   - Если клиент спрашивает про ДРУГУЮ СТОРОНУ (левую/правую), другой цвет, другой год, другую модификацию той же запчасти (например "Такой нету на левую сторону?", "А в чёрном цвете есть?", "А на модель 2020 года?") — это ТОЖЕ запрос наличия. Напиши: "Сейчас посмотрим и вернёмся с ответом!" и ОБЯЗАТЕЛЬНО добавь [NEEDS_MANAGER: Клиент спрашивает про вариацию запчасти]
...
10. БЛАГОДАРНОСТИ И ПРОЩАНИЯ:
   - Если клиент ВПЕРВЫЕ благодарит ("Спасибо", "Благодарю" и т.п.) — ответь коротко: "Пожалуйста! Обращайтесь, если будут вопросы."
   - Если ты УЖЕ попрощался/ответил на благодарность (в истории есть твоё "Пожалуйста" или "Обращайтесь"), а клиент пишет подтверждающие слова ("Хорошо", "Ок", "Понял", "Понятно", "Ладно", "Ясно", "Спасибо", "Спс", "Благодарю", "Угу", "Ага", "Да", "Окей", "Класс", "Супер", "Отлично", "Круто") — НЕ ОТВЕЧАЙ. Напиши ровно одно слово: [NO_RESPONSE]
   - Правило: если последнее твоё сообщение в истории содержит прощание/благодарность, а клиент пишет короткое подтверждение (1-3 слова) — [NO_RESPONSE]
   - Не начинай новую тему и не предлагай ничего дополнительного после прощания.
...
- Клиент спрашивает про другие запчасти, вариации (другая сторона, цвет, год, модификация) или наличие чего-то, чего нет в объявлении
...
Если эскалация не нужна — НЕ добавляй этот тег.`;
  ...
  try {
    ...
    // Check for NO_RESPONSE signal — bot decided not to reply
    if (responseText.includes('[NO_RESPONSE]') || responseText.trim() === '[NO_RESPONSE]') {
      return {
        text: '',
        needsManager: false,
      };
